...
import re
import shutil
from src.infrastructure.rules_engine import compile_venue_rules
import unicodedata
...
CITY_NAMES = {
    "kedzierzyn_kozle": "Kêdzierzyn-KoŸle",
    "opole": "Opole",
...
META_LABELS = [
    ...
    "Re¿yseria", "Scenografia", "Kostiumy", "Muzyka", "Œwiat³o",
    "Choreografia", "Asystentka re¿ysera", "Asystent re¿ysera",
    "Kierownictwo muzyczne", "Produkcja", "Kierownik produkcji",
    "Obsada", "Wystêpuj¹", "Wykonawcy", "Artyœci", "Prowadzenie",
...
def _process_event_description_to_html(ev: Any) -> str:
    ...
    if not raw_desc or len(str(raw_desc).strip()) < 5:
        return "<p>Brak szczegó³owego opisu wydarzenia.</p>"
    ...
    import re
    t = re.sub(r'([\w¹æê³ñóœŸ¿¥ÆÊ£ÑÓŒ¯0-9])\(', r'\1 (', t)
    t = re.sub(r'\)([\w¹æê³ñóœŸ¿¥ÆÊ£ÑÓŒ¯0-9])', r') \1', t)
    t = re.sub(r'([a-z¹æê³ñóœŸ¿])([A-Z¥ÆÊ£ÑÓŒ¯]{2,})', r'\1 \2', t)
    t = re.sub(r'([a-z¹æê³ñóœŸ¿])([A-Z¥ÆÊ£ÑÓŒ¯][a-z¹æê³ñóœŸ¿])', r'\1 \2', t)

    target = "wiêcej informacji"
...
class HTMLRenderer:
    ...
    def render_portal_hub(self, active_cities: List[Dict[str, str]], output_dir: str = "public"):
        ...
        print(f"[RENDERER] Strona g³ówna portalu: {hub_file}")
    ...
    def render_city(
        ...
        # 1. Czyszczenie i przygotowanie katalogu wydarzeñ
        ...
        def _write_single_event(ev: FullEventPage):
            ...
            compiled_rules = compile_venue_rules(place_obj, ev)
        ...
        # Równoleg³y zapis podstron wydarzeñ (Thread Pool)
        with ThreadPoolExecutor(max_workers=min(16, (os.cpu_count() or 4) * 2)) as executor:
            list(executor.map(_write_single_event, events))

        # 2. Preindeksacja powi¹zañ wydarzeñ do miejsc w O(N) zamiast zagnie¿d¿onego O(M * N)
        ...
        # 3. Renderowanie agendy g³ównej miasta
        BATCH_SIZE = 40
        ...
        print(f"[RENDERER] {city_name}: Wygenerowano {len(events)} podstron wydarzeñ i {rendered_places} wizytówek miejsc.")
