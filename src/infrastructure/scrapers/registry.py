...
def get_scrapers_for_city(city_tag: str, partner_id: str = "") -> List[BaseScraper]:
    ...
    # 1. Auto-discovery scraperów lokalnych ze zaktualizowanej ścieżki
    city_dir = SCRAPERS_DIR / normalized_tag
    local_classes = _discover_scraper_classes(f"src.infrastructure.scrapers.{normalized_tag}", city_dir)
    ...
    # 2. Auto-discovery scraperów ogólnopolskich ze zaktualizowanej ścieżki
    national_dir = SCRAPERS_DIR / "national"
    national_classes = _discover_scraper_classes("src.infrastructure.scrapers.national", national_dir)
